...
// P0..2 phone    - установка прописанных номеров, формат СМС "P0 79001112233", в рамках безопасности это может сделать или уже авторизованный номер, или при нажатом тормозе и выключенном двигателе 
...
// FactoryReset   - сброс на типовые настройки
...
#define PORT_OUTPUT_STARTER 9                       // d9 реле стартера
#define PORT_OUTPUT_SECPOWER 10                     // d10 реле цепи вторичного питания печка, фары и т.д.
#define PORT_OUTPUT_ENGINE 11                       // d11 реле зажигания
#define PORT_OUTPUT_IMMO 12                         // d12 реле обходчика иммобилайзера
#define PORT_OUTPUT_BUZZER 5                        // d5 пищалка (для тестов)
#define PORT_OUTPUT_LED_STATUS 13                   // d13 светодиод (для тестов)
#define PORT_OUTPUT_MODEM_BOOT 17                   // порт A3 (или 17) управление модемом, инверсия через транзитор
...
#define BALANCE_COMMAND "ATD#100#"                  // Команда запроса баланса. Для МТС-МСК выдает в ответ строчку типа Balance:123.45r
...
#define EEPROM_ADDRES_SMS_ON 10                     // хранение переменной sms_on = СМС уведомления о запуске включены 
#define EEPROM_ADDRES_EWT 11                        // хранение переменной engine_warm_time_setting EWT = время прогрева
...
const int MEASURE_COUNT = 10;                       // циклов периода измерения напряжения батареи
...
boolean sms_on = 0;                                 // СМС 0-выключены, 1-включены
...
String buf = "";
// Датчик температуры
OneWire  ds(PORT_INPUT_DS18B20);                    // on pin 10 (a 4.7K resistor is necessary)
//==================================================================================================================================================================
void setup() {
 Serial.begin(9600);                                //скорость порта
 //Serial.println("setup_function_start");
 //EEPROM.update(0, 255); //reset config;
 //eepromFactoryReset();
 eepromReadSettings();
 
 gsm.begin(9600); //скорость порта модема
 pinMode(PORT_OUTPUT_MODEM_BOOT, OUTPUT);           // порт управления загрузкой модема
 SetBootModemPin(1);
   
 // Настройка портов   
  pinMode(PORT_OUTPUT_STARTER, OUTPUT);                //стартер
  digitalWrite(PORT_OUTPUT_STARTER, LOW);              //выкл.
 
  pinMode(PORT_OUTPUT_ENGINE, OUTPUT);                 //зажигание
  digitalWrite(PORT_OUTPUT_ENGINE, LOW);               //выкл.
 
  pinMode(PORT_OUTPUT_SECPOWER, OUTPUT);               //вторичные цепи
  digitalWrite(PORT_OUTPUT_SECPOWER, LOW);             //выкл. 

  pinMode(PORT_OUTPUT_BUZZER, OUTPUT);                 //пищалка
  digitalWrite(PORT_OUTPUT_BUZZER, LOW);               //выкл.
...
if (engine_is_started) {                                                    //ждем окончания прогрева
    if (VBatt()<U1_START) Do_shutdown();                                    //проверяем что движок случайно не заглох
...
else { //если двигатель не запущен проверяем возможность запуска = есть неиспользованные попытки, генератор не работает и зажигание не включено
...
//==================================================================================================================================================================
void SetBootModemPin (boolean pin_status) { // устанавливает низкий 0 или высокий 1 уровень на входе BOOT модема
 if (pin_status==1) digitalWrite(PORT_OUTPUT_MODEM_BOOT, LOW);    // установить 1 на входе BOOT модема (0 на контроллере)
 if (pin_status==0) digitalWrite(PORT_OUTPUT_MODEM_BOOT, HIGH);   // установить 0 на входе BOOT модема (1 на контроллере)
}
//==================================================================================================================================================================
void ModemOFF() {
 SetBootModemPin(1);
...
//==================================================================================================================================================================
void GetPhoneFromStringBUF (String stg, int indexphone){ // Из строки buf c позиции строки stg устанавливает телефонный номер 11 знаков в ячейке indexphone
  int start0 = buf.indexOf(stg)+stg.length();
  int start1 = 0;
  int stop1 = 0;
  char ci;
  int i=0;
  for (i=start0; i<buf.length(); i++){            //ищем первую и последнюю цифру
    ci=buf.charAt(i);
  ...
  for (i=start1; i<stop1+1; i++) MasterPhone[indexphone][i-start1] = buf.charAt(i);   // копируем 11 ЦИФР  
...
void BeepNraz(int n){ //Зуммер сигналит N раз
  for (int i=0; i<n; i++){
  tone (PORT_OUTPUT_BUZZER, 1000, 100); 
  delay22(300); 
  }
}
//==================================================================================================================================================================
...
if (gsm.available()) { //если GSM модуль что-то послал нам, то
  ...
  index_master=255;
  for (int i=0; i<COUNT_PHONE; i++) if (buf.indexOf(MasterPhone[i])> -1) index_master=i; // присваиваем индекс если мастер номер
  ...
  // ПРИЕМ и РАЗБОР СМС С АВТОРИЗОВАННОГО ТЕЛЕФОНА, Можно сразу разбирать команды не определяя СМС это или нет, только для мастера
...
tmp="p"+String(i);
if (buf.indexOf(tmp)>-1){                                   // УСТАНОВИТЬ МАСТЕР НОМЕР i-ый  
  GetPhoneFromStringBUF(tmp, i);  
  EEPROM.put(EEPROM_ADDRES_MASTER0+i*12, MasterPhone[i]);   // записываем массив в EEPROM
  Serial.println("set MasterPhone"+String(i)+" = "+MasterPhone[i]); 
  }
}
eepromReadSettings();   
}
...
buf.toLowerCase();                                          // преобразовать в нижний регистр 
  if (buf.indexOf("start")>-1) {                            // ЗАПУСТИТЬ ДВИГАТЕЛЬ
    ...
    Do_shutdown();
    delay22(500);
    if (sms_on) SendSMS(Info());
    } 
  else if (buf.indexOf("list")>-1){                         //отправить по СМС все мастер телефоны
    tmp="Phones: "; for (int i=0; i<COUNT_PHONE; i++) {tmp+=MasterPhone[i]; if (i<COUNT_PHONE-1) tmp+=", ";}
    SendSMS (tmp);
    Serial.println(tmp);
    }
    ...
    sms_on = 1;
    EEPROM.put(EEPROM_ADDRES_SMS_ON, (int)sms_on);           // записали число по адресу 10
    SendSMS ("SMS ON applied");   
    }
  else if (buf.indexOf("smsoff")>-1){
    Serial.println("SMS OFF command");                       // выключить ответные СМС
    sms_on = 0;
    EEPROM.put(EEPROM_ADDRES_SMS_ON, (int)sms_on);           // записали число по адресу 10
    SendSMS ("SMS OFF applied");   
    }
  else if (buf.indexOf("factoryreset")>-1){                  // перезагрузка данных
    ...
    EEPROM.put(EEPROM_ADDRES_EWT, engine_warm_time_setting);  // сколько времение в минутах прогревать двигатель  
    Serial.println("set engine_warm_time_setting = "+String(engine_warm_time_setting)); 
    if (engine_warm_time_setting>0) SendSMS ("Engine warm time = "+String(engine_warm_time_setting));
      else SendSMS ("Engine warm Time = AUTO");
    }
...
float ttemp = Temp();  
String strtmp = "";
if (engine_is_started==0) 
  strtmp+="Is Not Running\n";                   //"Engine is NOT Running\n"; 
  else strtmp+="Is Running\n";                  //"Engine Is Running\n";
strtmp+= "Batt: "+String(VBatt())+"V\n";        //"Voltage: "+String(VBatt())+"V\n";
strtmp+="WarmTime: ";                           //"EngineWarmTime: "; 
if (engine_warm_time_setting>0) 
  strtmp+=String(engine_warm_time_setting)+"min."+'\n'; 
  else strtmp+="Auto\n";
if (engine_is_started==1) 
  strtmp+="Time Left: "+ String(engine_warm_time_local-(millis()-last_start_time)/60000)+"min.\n"; 
if (ttemp>-100)         // если датчика нет то функция возврящает -100 и значение не выводится
  strtmp+="Temp: "+ String(ttemp)+"C\n";                             
if (sms_on) 
  strtmp+="SMS ON\n"; 
  else strtmp+="SMS OFF\n";
strtmp+="UpTime: " + String(millis()/3600000) + "Hr";
return strtmp;
}
//========================================================================================================================================================
...
digitalWrite(PORT_OUTPUT_IMMO, HIGH);      Serial.println("IMMO ON");         //включаем обходчик иммобилайзера
delay22(100);
digitalWrite(PORT_OUTPUT_ENGINE, HIGH);    Serial.println("ENGINE ON");       //включаем зажигание 
delay22(500);
digitalWrite(PORT_OUTPUT_SECPOWER, HIGH);  Serial.println("SECPOWER ON");     // включаем печку фары и тд
delay22(3000);                                                                // ждем, чтобы бензонасос набрал давление
digitalWrite(PORT_OUTPUT_SECPOWER, LOW);   Serial.println("SECPOWER OFF");    // выключаем печку фары и тд.
...
digitalWrite(PORT_OUTPUT_STARTER, HIGH);   Serial.println("STARTER ON");      // включаем стартер 
BeepNraz(2); //ТЕСТ
...
digitalWrite(PORT_OUTPUT_STARTER, LOW);    Serial.println("STARTER OFF");     // отключаем стартер 
...
if (VBatt()>U1_START) {                                                       // смотрим, что  зарядка пошла  
  ...
  digitalWrite(PORT_OUTPUT_SECPOWER, HIGH); Serial.println("SECPOWER ON");    //включаем печку фары итд
  ...
  if (sms_on==1) SendSMS(Info());
  }
else{ // не завелся
  digitalWrite(PORT_OUTPUT_SECPOWER, LOW);   Serial.println("SECPOWER OFF");  //выключаем печку фары и тд
  delay22(500);                              Serial.println("delay(500)");
  digitalWrite(PORT_OUTPUT_ENGINE, LOW);     Serial.println("ENGINE OFF");    //выключаем зажигание чтобы разблокировать реле стартера
  digitalWrite(PORT_OUTPUT_IMMO, LOW);       Serial.println("IMMO OFF");      //выключаем обходчик иммобилайзера
  engine_is_started = 0;                                                      //Запоминаем что движок не запущен
  Serial.println("NOT STARTED\n"+Info());
  if (sms_on==1 && left_start_try==1) SendSMS(Info());                        // отправка СМС о неудачном запуске
...
//========================================================================================================================================================
void Do_shutdown (){
if (digitalRead(PORT_OUTPUT_SECPOWER)==HIGH){
  digitalWrite(PORT_OUTPUT_SECPOWER, LOW);     //выключаем печку фары итд 
  Serial.println("SECPOWER LOW");   
  delay22(100);  
}
if (digitalRead(PORT_OUTPUT_ENGINE)==HIGH){
  digitalWrite(PORT_OUTPUT_ENGINE, LOW);       //выключаем зажигание
...
if (analogRead(PORT_INPUT_ENGINE)*MEASURE_RATIO>VBATT_LOW) return 1;  
else return 0;  
}
//=======================================================================================================================================================
boolean DetectStop(){
if (analogRead(PORT_INPUT_STOP)*MEASURE_RATIO>VBATT_LOW) return 1; // если нажат стоп
else return 0;  
}
//========================================================================================================================================================
...
EEPROM.put(EEPROM_ADDRES_SMS_ON, 0);        // sms_on=0 записали число 0 по адресу 10
EEPROM.put(EEPROM_ADDRES_EWT, 10);          // engine_warm_time_setting=10 записали по адресу 11

byte raw[2];                                // запись времени прокрутки стартера starter_time_setting=1000
(int&)raw = 1000;
for(byte i = 0; i < 2; i++) EEPROM.write(EEPROM_ADDRES_STARTER+i, raw[i]);
// стирание телефонных номеров
for (int i=0; i<COUNT_PHONE; i++) EEPROM.put(EEPROM_ADDRES_MASTER0+i*12, "");
}
//========================================================================================================================================================
void eepromReadSettings(){
// включены ли СМС  
EEPROM.get(EEPROM_ADDRES_SMS_ON, sms_on);                                             
sms_on = constrain(sms_on, 0, 1);
if (sms_on) Serial.println("sms_on = 1"); else Serial.println("sms_on = 0");
...
for (int i=0; i<COUNT_PHONE; i++) {                                                  
  ...
  MasterPhone[i][11]='\0';
  Serial.println("MasterPhone["+String(i)+"] = "+MasterPhone[i]);                      //вывести все мастер телефоны
  }
}
//========================================================================================================================================================
float Temp () { //получение температуры с датчика
 byte i; byte type_s; byte data[12]; byte addr[8]; float celsius; int cnt=3;
 while ( !ds.search(addr) && cnt>0) {ds.reset_search(); cnt--;} if (cnt==0) return -100; //3 попытки определения адреса датчика
 if (OneWire::crc8(addr, 7) != addr[7]) return -100;// CRC is not valid!
 switch (addr[0]) {case 0x10: type_s = 1; break; case 0x28: type_s = 0; break; case 0x22: type_s = 0; break; default: return -100; } 
 ds.reset(); ds.select(addr); ds.write(0x44); //ds.write(0x44, 1); delay(1000); // для паразитного питания датчика
 ds.reset(); ds.select(addr); ds.write(0xBE); 
 for ( i = 0; i < 9; i++) data[i] = ds.read(); int16_t raw = (data[1] << 8) | data[0];
 if(type_s){raw=raw<<3;if(data[7]==0x10)raw=(raw&0xFFF0)+12-data[6];}
 else{byte cfg=(data[4]&0x60);if(cfg==0x00)raw=raw&~7;else if(cfg==0x20)raw=raw&~3;else if(cfg==0x40)raw=raw&~1;}
